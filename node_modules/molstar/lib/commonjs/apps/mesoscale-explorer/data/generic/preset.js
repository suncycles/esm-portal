...
exports.getTransforms = exports.createGenericHierarchy = void 0;
...
async function createGenericHierarchy(plugin, file) {
    ...
    async function addGroup(g, cell, parent) {
        const group = await state.build()
            .to(cell)
            .apply(state_1.MesoscaleGroup, { ...groupParams, index: undefined, tag: `${g.root}:${g.id}`, label: g.label || g.id, color: { type: 'custom', value: names_1.ColorNames.white, variability: 20, shift: 0, lightness: 0, alpha: 1, emissive: 0 } }, { tags: [`group:${g.root}:${g.id}`, g.root === parent ? `${g.root}:` : `${g.root}:${parent}`], state: { isCollapsed: true, isHidden: groupParams.hidden } })
            .commit();
        if (g.children) {
    ...
    for (const r of manifest.roots) {
        const root = await state.build()
            .toRoot()
            .apply(state_1.MesoscaleGroup, { ...groupParams, root: true, index: -1, tag: `${r.id}:`, label: r.label || r.id, color: { type: 'custom', value: names_1.ColorNames.white, variability: 20, shift: 0, lightness: 0, alpha: 1, emissive: 0 } }, { tags: `group:${r.id}:`, state: { isCollapsed: false, isHidden: groupParams.hidden } })
            .commit();
        if (r.children) {
    ...
    await state.transaction(async () => {
        try {
            ...
            for (const ent of manifest.entities) {
                ...
                const color = names_1.ColorNames.skyblue;
                ...
                if (['gro', 'cif', 'mmcif', 'mcif', 'bcif', 'pdb', 'ent', 'xyz', 'mol', 'sdf', 'sd', 'mol2'].includes(info.ext)) {
                    ...
                    build = build
                        ...
                        .apply(model_2.StructureFromGeneric, { instances, label })
            ...
            const values = { type: 'group-generate', value: names_1.ColorNames.white, lightness: 0, alpha: 1 };
            await (0, state_1.updateColors)(plugin, values, rootId, '');
        }
        catch (e) {
            console.error(e);
            plugin.log.error(e);
