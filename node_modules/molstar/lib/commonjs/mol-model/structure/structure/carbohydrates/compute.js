...
const combination_1 = require("../../../../mol-data/util/combination");
...
const C = (0, types_1.ElementSymbol)('C'), O = (0, types_1.ElementSymbol)('O');
const SugarRingFps = [
...
function getDirection(direction, unit, index, center) {
    ...
    linear_algebra_1.Vec3.normalize(direction, linear_algebra_1.Vec3.sub(direction, center, c.position(index, direction)));
    return direction;
}
...
function filterFusedRings(unitRings, rings) {
    ...
    const ringCombinations = (0, combination_1.combinations)((0, array_1.fillSerial)(new Array(rings.length)), 2);
...
function computeCarbohydrates(structure) {
    ...
    function fixTerminalLinkDirection(iA, indexB, unitB) {
        ...
        linear_algebra_1.Vec3.sub(geo.direction, c.position(unitB.elements[indexB], tmpV), geo.center);
        ...
        while (chainIt.hasNext) {
            ...
            while (residueIt.hasNext) {
                ...
                for (let j = 0, jl = sugarRings.length; j < jl; ++j) {
                    ...
                    const ma = principal_axes_1.PrincipalAxes.calculateMomentsAxes((0, util_1.getPositions)(unit, ringAtoms));
                    const a = principal_axes_1.PrincipalAxes.calculateNormalizedAxes(ma);
                    const center = linear_algebra_1.Vec3.copy((0, linear_algebra_1.Vec3)(), a.origin);
                    const normal = linear_algebra_1.Vec3.copy((0, linear_algebra_1.Vec3)(), a.dirC);
                    const direction = getDirection((0, linear_algebra_1.Vec3)(), unit, anomericCarbon, center);
                    linear_algebra_1.Vec3.orthogonalize(direction, normal, direction);
                    const ringAltId = rings_1.UnitRing.getAltId(unit, ringAtoms);
                ...
                const ringCombinations = (0, combination_1.combinations)((0, array_1.fillSerial)(new Array(sugarRings.length)), 2);
