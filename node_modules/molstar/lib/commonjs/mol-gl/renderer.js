...
(function (Renderer) {
    function create(ctx, props = {}) {
        ...
        const renderDepth = (group, camera, depthTexture) => {
            if (debug_1.isTimingMode)
        ...
        const renderMarkingDepth = (group, camera, depthTexture) => {
            if (debug_1.isTimingMode)
        ...
        const renderBlendedTransparent = (group, camera, depthTexture) => {
            var _a, _b;
            if (debug_1.isTimingMode)
        ...
        const renderBlendedVolume = (group, camera, depthTexture) => {
            if (debug_1.isTimingMode)
            ...
            for (let i = 0, il = renderables.length; i < il; ++i) {
                const r = renderables[i];
                if (r.values.dGeometryType.ref.value === 'directVolume') {
                    renderObject(r, 'color', 3 /* Flag.BlendedVolume */);
                }
            }
            if (debug_1.isTimingMode)
        ...
        const renderDpoitVolume = (group, camera, depthTexture) => {
            if (debug_1.isTimingMode)
        ...
        return {
            clear: (toBackgroundColor, ignoreTransparentBackground) => {
                state.enable(gl.SCISSOR_TEST);
                state.enable(gl.DEPTH_TEST);
                state.colorMask(true, true, true, true);
                state.depthMask(true);
