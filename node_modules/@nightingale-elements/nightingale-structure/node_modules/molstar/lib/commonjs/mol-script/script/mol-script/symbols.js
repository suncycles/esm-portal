...
var symbol_1 = require("../../language/symbol");
var symbol_table_1 = require("../../language/symbol-table");
var type_1 = require("../../language/type");
var structure_query_1 = require("../../language/symbol-table/structure-query");
var builder_1 = require("../../language/builder");
var script_macro_1 = require("./script-macro");
...
function Macro(symbol, translate) {
    ...
    symbol.id = "molscript-macro.".concat(symbol.info.name);
    return { kind: 'macro', symbol: symbol, translate: translate, aliases: tslib_1.__spreadArray([symbol.info.name], aliases, true) };
}
...
exports.SymbolTable = [
    ...
    [
        ...
        [
            ...
            }), structure_query_1.Types.ElementSelection, 'A selection of singleton atom sets.'), function (args) { return builder_1.MolScriptBuilder.struct.generator.atomGroups({ 'atom-test': (0, script_macro_1.tryGetArg)(args, 0, true) }); }),
            ...
            }), structure_query_1.Types.ElementSelection, 'A selection of atom sets grouped by residue.'), function (args) { return builder_1.MolScriptBuilder.struct.generator.atomGroups({
            ...
            }), structure_query_1.Types.ElementSelection, 'A selection of atom sets grouped by chain.'), function (args) { return builder_1.MolScriptBuilder.struct.generator.atomGroups({
...
var normalized = (function () {
    ...
    var namedArgs = generic_1.UniqueArray.create();
    var constants = generic_1.UniqueArray.create();
    for (var _a = 0, list_1 = list; _a < list_1.length; _a++) {
        ...
        for (var _b = 0, _c = s.aliases; _b < _c.length; _b++) {
            ...
            symbolList.push([a, s]);
            if (symbolMap[a])
                throw new Error("Alias '".concat(a, "' already in use."));
            symbolMap[a] = s;
        }
        var args = s.symbol.args;
        if (args.kind !== 'dictionary') {
            if (args.type.kind === 'oneof') {
                type_1.Type.oneOfValues(args.type).forEach(function (v) { return generic_1.UniqueArray.add(constants, v, v); });
            }
            continue;
        }
        for (var _d = 0, _e = Object.keys(args.map); _d < _e.length; _d++) {
            ...
            if (arg.type.kind === 'oneof') {
                type_1.Type.oneOfValues(arg.type).forEach(function (v) { return generic_1.UniqueArray.add(constants, v, v); });
            }
        }
    }
...
function substSymbols(expr) {
    ...
    if (expression_1.Expression.isSymbol(expr)) {
        if (!exports.SymbolMap[expr.name])
            return expr;
        var s = exports.SymbolMap[expr.name];
        if (s.kind === 'alias')
            return expression_1.Expression.Symbol(exports.SymbolMap[expr.name].symbol.id);
        throw s.translate([]);
    }
    var isMacro = expression_1.Expression.isSymbol(expr.head) && !!exports.SymbolMap[expr.head.name] && exports.SymbolMap[expr.head.name].kind === 'macro';
    ...
    if (isMacro) {
        var macro = exports.SymbolMap[expr.head.name];
        if (macro.kind !== 'macro')
