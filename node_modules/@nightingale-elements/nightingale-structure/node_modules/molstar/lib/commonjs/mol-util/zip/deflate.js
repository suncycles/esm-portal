    ...
    for (; i < end; i++) {
        ...
        if (cvrd <= i) {
            ...
            if (mch !== 0) {
                ...
                var lgi = _goodIndex(len, util_1.U.of0);
                util_1.U.lhst[257 + lgi]++;
                var dgi = _goodIndex(dst, util_1.U.df0);
                util_1.U.dhst[dgi]++;
...
function _deflateRaw(runtime, data, out, opos, lvl) {
...
function _writeBlock(BFINAL, lits, li, ebits, data, o0, l0, out, pos) {
    ...
    var fxdSize = ebits + contSize(util_1.U.fltree, util_1.U.lhst) + contSize(util_1.U.fdtree, util_1.U.dhst);
    var dynSize = ebits + contSize(util_1.U.ltree, util_1.U.lhst) + contSize(util_1.U.dtree, util_1.U.dhst);
...
*/
function getTrees() {
    var ML = (0, huffman_1._hufTree)(util_1.U.lhst, util_1.U.ltree, 15);
    var MD = (0, huffman_1._hufTree)(util_1.U.dhst, util_1.U.dtree, 15);
    var lset = [];
    var numl = _lenCodes(util_1.U.ltree, lset);
    var dset = [];
    var numd = _lenCodes(util_1.U.dtree, dset);
    for (var i = 0; i < lset.length; i += 2)
        util_1.U.ihst[lset[i]]++;
    for (var i = 0; i < dset.length; i += 2)
        util_1.U.ihst[dset[i]]++;
    var MH = (0, huffman_1._hufTree)(util_1.U.ihst, util_1.U.itree, 7);
    var numh = 19;
