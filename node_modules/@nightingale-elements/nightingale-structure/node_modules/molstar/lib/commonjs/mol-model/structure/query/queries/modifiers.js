...
exports.surroundingLigands = exports.includeConnected = exports.expandProperty = exports.union = exports.exceptBy = exports.intersectBy = exports.querySelection = exports.includeSurroundings = exports.wholeResidues = void 0;
var tslib_1 = require("tslib");
var int_1 = require("../../../../mol-data/int");
var selection_1 = require("../selection");
var builders_1 = require("../utils/builders");
var unique_subset_builder_1 = require("../../structure/util/unique-subset-builder");
var structure_set_1 = require("../utils/structure-set");
var generic_1 = require("../../../../mol-data/generic");
var element_1 = require("../../structure/element");
var mmcif_1 = require("../../../../mol-model-formats/structure/mmcif");
var residue_set_1 = require("../../model/properties/utils/residue-set");
var properties_1 = require("../../structure/properties");
...
function getWholeResidues(ctx, source, structure) {
    ...
    for (var _a = 0, _b = structure.units; _a < _b.length; _a++) {
        ...
        var residuesIt = int_1.Segmentation.transientSegments(residueAtomSegments, elements);
...
function wholeResidues(query) {
    return function query_wholeResidues(ctx) {
        ...
        if (selection_1.StructureSelection.isSingleton(inner)) {
            ...
            var builder = new builders_1.UniqueStructuresBuilder(ctx.inputStructure);
...
function getIncludeSurroundings(ctx, source, structure, params) {
    var builder = new unique_subset_builder_1.StructureUniqueSubsetBuilder(source);
...
function includeSurroundings(query, params) {
    return function query_includeSurroundings(ctx) {
        ...
        if (params.elementRadius) {
            var prms = tslib_1.__assign(tslib_1.__assign({}, params), { elementRadius: params.elementRadius, elementRadiusClosure: createElementRadiusFn(ctx, params.elementRadius), sourceMaxRadius: findStructureRadius(ctx, params.elementRadius) });
            if (selection_1.StructureSelection.isSingleton(inner)) {
                var surr = getIncludeSurroundingsWithRadius(ctx, ctx.inputStructure, inner.structure, prms);
                var ret = selection_1.StructureSelection.Singletons(ctx.inputStructure, surr);
                return ret;
            }
            else {
                var builder = new builders_1.UniqueStructuresBuilder(ctx.inputStructure);
...
function querySelection(selection, query) {
    return function query_querySelection(ctx) {
        ...
        selection_1.StructureSelection.forEach(targetSel, function (s, sI) {
...
function intersectBy(query, by) {
    return function query_intersectBy(ctx) {
        ...
        var unionBy = selection_1.StructureSelection.unionStructure(bySel);
        var ret = selection_1.StructureSelection.UniqueBuilder(ctx.inputStructure);
        selection_1.StructureSelection.forEach(selection, function (s, sI) {
            var ii = (0, structure_set_1.structureIntersect)(unionBy, s);
...
function exceptBy(query, by) {
    return function query_exceptBy(ctx) {
        ...
        if (selection_1.StructureSelection.structureCount(bySel) === 0)
            return selection;
        var subtractBy = selection_1.StructureSelection.unionStructure(bySel);
        var ret = selection_1.StructureSelection.UniqueBuilder(ctx.inputStructure);
        selection_1.StructureSelection.forEach(selection, function (s, sI) {
            var diff = (0, structure_set_1.structureSubtract)(s, subtractBy);
...
function expandProperty(query, property) {
    return function query_expandProperty(ctx) {
        ...
        var builders = [];
        ctx.pushCurrentElement();
        selection_1.StructureSelection.forEach(src, function (s, sI) {
        ...
        ctx.popCurrentElement();
        var ret = selection_1.StructureSelection.UniqueBuilder(ctx.inputStructure);
        for (var _c = 0, builders_2 = builders; _c < builders_2.length; _c++) {
            var b = builders_2[_c];
...
function includeConnected(_a) {
    ...
    return function query_includeConnected(ctx) {
        var builder = selection_1.StructureSelection.UniqueBuilder(ctx.inputStructure);
        var src = query(ctx);
        ctx.pushCurrentBond();
...
function expandConnected(ctx, structure) {
    ...
    var builder = new unique_subset_builder_1.StructureUniqueSubsetBuilder(inputStructure);
    ...
    for (var _a = 0, _b = structure.units; _a < _b.length; _a++) {
        ...
        for (var i = 0, _i = unit.elements.length; i < _i; i++) {
            ...
            var aIndex = int_1.SortedArray.indexOf(inputUnitA.elements, unit.elements[i]);
...
function surroundingLigands(_a) {
    ...
    return function query_surroundingLigands(ctx) {
        ...
        var l = element_1.StructureElement.Location.create(surroundings);
        var includedPrdChains = new Map();
        var componentResidues = new residue_set_1.ResidueSet({ checkOperator: true });
        for (var _a = 0, _b = surroundings.units; _a < _b.length; _a++) {
            ...
            while (chainsIt.hasNext) {
                ...
                var asym_id = properties_1.StructureProperties.chain.label_asym_id(l);
                var op_name = properties_1.StructureProperties.unit.operator_name(l);
                ...
                var entityType = properties_1.StructureProperties.entity.type(l);
        ...
        var components = (0, structure_set_1.structureUnion)(ctx.inputStructure, [builder.getStructure(), inner]);
        // add water
        if (includeWater) {
            var finalBuilder = new unique_subset_builder_1.StructureUniqueSubsetBuilder(ctx.inputStructure);
...
var _entity_type = properties_1.StructureProperties.entity.type;
...
var StructConnGraph = /** @class */ (function () {
    ...
    StructConnGraph.prototype.addEdge = function (a, b) {
        var al = residue_set_1.ResidueSet.getLabel(a);
        var bl = residue_set_1.ResidueSet.getLabel(b);
        this.addVertex(a, al);
        this.addVertex(b, bl);
    ...
    StructConnGraph.prototype.addComponent = function (start, set) {
        var startLabel = residue_set_1.ResidueSet.getLabel(start);
        ...
        while (stack.length > 0) {
            ...
            for (var _a = 0, _b = this.edges.get(a); _a < _b.length; _a++) {
                ...
                if (u.operator_name === v.operator_name) {
                    set.add(tslib_1.__assign(tslib_1.__assign({}, v), { operator_name: start.operator_name }));
                }
                else {
                    set.add(v);
                }
