...
exports.computeCarbohydrates = void 0;
var tslib_1 = require("tslib");
var int_1 = require("../../../../mol-data/int");
var combination_1 = require("../../../../mol-data/util/combination");
...
var rings_1 = require("../unit/rings");
var util_2 = require("../../../../mol-data/util");
var C = (0, types_1.ElementSymbol)('C'), O = (0, types_1.ElementSymbol)('O');
var SugarRingFps = [
    rings_1.UnitRing.elementFingerprint([C, C, C, O]),
    rings_1.UnitRing.elementFingerprint([C, C, C, C, O]),
    rings_1.UnitRing.elementFingerprint([C, C, C, C, C, O]),
    rings_1.UnitRing.elementFingerprint([C, C, C, C, C, C, O]),
];
...
function getDirection(direction, unit, index, center) {
    ...
    linear_algebra_1.Vec3.normalize(direction, linear_algebra_1.Vec3.sub(direction, center, position(index, direction)));
    return direction;
}
...
function filterFusedRings(unitRings, rings) {
    ...
    var ringCombinations = (0, combination_1.combinations)((0, array_1.fillSerial)(new Array(rings.length)), 2);
    for (var i = 0, il = ringCombinations.length; i < il; ++i) {
        var rc = ringCombinations[i];
        var r0 = all[rings[rc[0]]], r1 = all[rings[rc[1]]];
        if (int_1.SortedArray.areIntersecting(r0, r1) &&
            rings_1.UnitRing.getAltId(unit, r0) === rings_1.UnitRing.getAltId(unit, r1)) {
...
function computeCarbohydrates(structure) {
    ...
    if (structure.models.reduce(function (a, v) { return a + v.properties.saccharideComponentMap.size; }, 0) === 0)
        return data_1.EmptyCarbohydrates;
    var links = [];
    var terminalLinks = [];
    var elements = [];
    var partialElements = [];
    ...
    function fixLinkDirection(iA, iB) {
        linear_algebra_1.Vec3.sub(elements[iA].geometry.direction, elements[iB].geometry.center, elements[iA].geometry.center);
        linear_algebra_1.Vec3.normalize(elements[iA].geometry.direction, elements[iA].geometry.direction);
    }
    ...
    function fixTerminalLinkDirection(iA, indexB, unitB) {
        ...
        linear_algebra_1.Vec3.sub(geo.direction, pos(unitB.elements[indexB], tmpV), geo.center);
        linear_algebra_1.Vec3.normalize(geo.direction, geo.direction);
    }
        ...
        while (chainIt.hasNext) {
            ...
            while (residueIt.hasNext) {
                ...
                if (!saccharideComp)
                    continue;
                if (!sugarResidueMap) {
                    sugarResidueMap = rings_1.UnitRings.byFingerprintAndResidue(rings, SugarRingFps);
                }
                ...
                for (var j = 0, jl = sugarRings.length; j < jl; ++j) {
                    ...
                    var ma = principal_axes_1.PrincipalAxes.calculateMomentsAxes((0, util_1.getPositions)(unit, ringAtoms));
                    var a = principal_axes_1.PrincipalAxes.calculateNormalizedAxes(ma);
                    var center = linear_algebra_1.Vec3.copy((0, linear_algebra_1.Vec3)(), a.origin);
                    var normal = linear_algebra_1.Vec3.copy((0, linear_algebra_1.Vec3)(), a.dirC);
                    var direction = getDirection((0, linear_algebra_1.Vec3)(), unit, anomericCarbon, center);
                    linear_algebra_1.Vec3.orthogonalize(direction, normal, direction);
                    var ringAltId = rings_1.UnitRing.getAltId(unit, ringAtoms);
                ...
                var ringCombinations = (0, combination_1.combinations)((0, array_1.fillSerial)(new Array(sugarRings.length)), 2);
                for (var j = 0, jl = ringCombinations.length; j < jl; ++j) {
                    ...
                    if (graph_1.IntAdjacencyGraph.areVertexSetsConnected(unit.bonds, r0, r1, 3)) {
    ...
    for (var i = 0, il = elements.length; i < il; ++i) {
        ...
        for (var j = i + 1; j < il; ++j) {
            ...
            if (graph_1.IntAdjacencyGraph.areVertexSetsConnected(unit.bonds, rA, rB, 3)) {
                // TODO handle better, for now fix both directions as it is unclear where the C1 atom is
                //      would need to know the path connecting the two rings
        ...
        structure.interUnitBonds.getConnectedUnits(unit.id).forEach(function (pairBonds) {
            pairBonds.connectedIndices.forEach(function (indexA) {
                pairBonds.getEdges(indexA).forEach(function (_a) {
                    ...
                    if (!types_1.BondType.isCovalent(props.flag))
                        return;
    ...
    return tslib_1.__assign({ links: links, terminalLinks: terminalLinks, elements: elements, partialElements: partialElements }, buildLookups(elements, links, terminalLinks));
}
exports.computeCarbohydrates = computeCarbohydrates;
function buildLookups(elements, links, terminalLinks) {
    function key(unit, element) {
        return (0, util_2.cantorPairing)(unit.id, element);
    }
    function getIndices(map, unit, index) {
        ...
        if (il !== undefined) {
            for (var _i = 0, il_1 = il; _i < il_1.length; _i++) {
                var i = il_1[_i];
                (0, array_1.arraySetAdd)(indices, i);
            }
        }
        return indices;
    }
