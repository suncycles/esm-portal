...
var hierarchy_1 = require("../atomic/hierarchy");
...
var _tempResidueKey = hierarchy_1.AtomicIndex.EmptyResidueKey();
var Index = /** @class */ (function () {
    ...
    Index.prototype.findResidueInsertion = function (key) {
        ...
        if (rm.has(id))
            return rm.get(id);
        var idx = int_1.SortedArray.findPredecessorIndex(this.map.label_seq_id, key.label_seq_id);
        var start = hierarchy_1.AtomicHierarchy.chainStartResidueIndex(this.map.segments, cI);
        if (idx < start)
            return start;
        var end = hierarchy_1.AtomicHierarchy.chainEndResidueIndexExcl(this.map.segments, cI) - 1;
        if (idx >= end)
            return end;
        return idx;
    };
...
function getAtomicIndex(data, entities, segments) {
    ...
    var chainsIt = int_1.Segmentation.transientSegments(segments.chainAtomSegments, atomSet);
    while (chainsIt.hasNext) {
        ...
        var residuesIt = int_1.Segmentation.transientSegments(segments.residueAtomSegments, atomSet, chainSegment);
