...
(function (StructureSymmetry) {
    function buildAssembly(structure, asmName) {
        var _this = this;
        return mol_task_1.Task.create('Build Assembly', function (ctx) { return tslib_1.__awaiter(_this, void 0, void 0, function () {
            ...
            return tslib_1.__generator(this, function (_e) {
                ...
                assembly = model_1.Symmetry.findAssembly(models[0], asmName);
                if (!assembly)
                    throw new Error("Assembly '".concat(asmName, "' is not defined."));
                coordinateSystem = geometry_1.SymmetryOperator.create(assembly.id, linear_algebra_1.Mat4.identity(), { assembly: { id: assembly.id, operId: 0, operList: [] } });
                assembler = structure_1.Structure.Builder({
                    coordinateSystem: coordinateSystem,
                    label: structure.label,
                    dynamicBonds: structure.dynamicBonds
                });
                queryCtx = new query_1.QueryContext(structure);
                for (_i = 0, _a = assembly.operatorGroups; _i < _a.length; _i++) {
                    ...
                    units = query_1.StructureSelection.unionStructure(selection).units;
    ...
    function buildSymmetryAssembly(structure, generators, symmetry) {
        var _this = this;
        return mol_task_1.Task.create('Build Symmetry Assembly', function (ctx) { return tslib_1.__awaiter(_this, void 0, void 0, function () {
            ...
            return tslib_1.__generator(this, function (_g) {
                ...
                modelCenter = (0, linear_algebra_1.Vec3)();
                assembler = structure_1.Structure.Builder({
                    label: structure.label,
    ...
    function builderSymmetryMates(structure, radius) {
        return mol_task_1.Task.create('Find Symmetry Mates', function (ctx) { return findMatesRadius(ctx, structure, radius); });
    }
    StructureSymmetry.builderSymmetryMates = builderSymmetryMates;
    function buildSymmetryRange(structure, ijkMin, ijkMax) {
        return mol_task_1.Task.create('Build Symmetry', function (ctx) { return findSymmetryRange(ctx, structure, ijkMin, ijkMax); });
    }
    ...
    function buildNcs(structure) {
        return mol_task_1.Task.create('Build NCS', function (ctx) { return _buildNCS(ctx, structure); });
    }
    ...
    function computeTransformGroups(s) {
        ...
        for (var _b = 0, _c = groups.groups; _b < _c.length; _b++) {
            var eqUnits = _c[_b];
            ret.push(unit_1.Unit.SymmetryGroup(eqUnits.map(function (id) { return s.unitMap.get(id); })));
        }
        return ret;
    }
...
exports.StructureSymmetry = StructureSymmetry;
function getSelector(asymIds) {
    return query_1.Queries.generators.atoms({ chainTest: query_1.Queries.pred.and(query_1.Queries.pred.eq(function (ctx) { return properties_1.StructureProperties.unit.operator_name(ctx.element); }, geometry_1.SymmetryOperator.DefaultName), query_1.Queries.pred.inSet(function (ctx) { return properties_1.StructureProperties.chain.label_asym_id(ctx.element); }, asymIds)) });
}
function getOperatorsForIndex(symmetry, index, i, j, k, modelCenter) {
    ...
    var symOp = geometry_1.Spacegroup.getSymmetryOperatorRef(spacegroup, index, i, j, k, ref);
    if (ncsOperators && ncsOperators.length) {
        for (var u = 0, ul = ncsOperators.length; u < ul; ++u) {
            var ncsOp = ncsOperators[u];
            var matrix = linear_algebra_1.Mat4.mul((0, linear_algebra_1.Mat4)(), symOp.matrix, ncsOp.matrix);
            var operator = geometry_1.SymmetryOperator.create("".concat(symOp.name, " ").concat(ncsOp.name), matrix, {
...
function assembleOperators(structure, operators) {
    var assembler = structure_1.Structure.Builder({
        label: structure.label,
        dynamicBonds: structure.dynamicBonds
    });
...
function _buildNCS(ctx, structure) {
    return tslib_1.__awaiter(this, void 0, void 0, function () {
        ...
        return tslib_1.__generator(this, function (_a) {
            ...
            symmetry = symmetry_1.ModelSymmetry.Provider.get(models[0]);
            if (!symmetry)
    ...
    return tslib_1.__awaiter(this, void 0, void 0, function () {
        ...
        return tslib_1.__generator(this, function (_a) {
            ...
            if (geometry_1.SpacegroupCell.isZero(spacegroup.cell))
                return [2 /*return*/, structure];
            modelCenter = model_1.Model.getCenter(models[0]);
