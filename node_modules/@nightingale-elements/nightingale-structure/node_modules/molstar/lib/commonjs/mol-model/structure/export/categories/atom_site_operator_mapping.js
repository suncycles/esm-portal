...
exports.AtomSiteOperatorMappingSchema = exports.AtomSiteOperatorMappingCategoryName = exports.atom_site_operator_mapping = void 0;
...
var Fields = cif_1.CifWriter.fields()
...
function getEntries(ctx) {
    ...
    for (var _i = 0, _a = ctx.structures; _i < _a.length; _i++) {
        var s = _a[_i];
        var l = structure_1.StructureElement.Location.create(s);
        for (var _b = 0, _c = s.units; _b < _c.length; _b++) {
            ...
            while (chainsIt.hasNext) {
                ...
                var label_asym_id = structure_1.StructureProperties.chain.label_asym_id(l);
                var key = "".concat(label_asym_id).concat(operator.suffix);
                if (existing.has(key))
                    continue;
                existing.add(key);
                var auth_asym_id = structure_1.StructureProperties.chain.label_asym_id(l);
