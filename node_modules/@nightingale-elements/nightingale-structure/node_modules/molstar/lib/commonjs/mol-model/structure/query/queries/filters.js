...
exports.isConnectedTo = exports.within = exports.areIntersectedBy = exports.withSameAtomProperties = exports.getCurrentStructureProperties = exports.first = exports.pick = void 0;
var set_1 = require("../../../../mol-util/set");
var structure_1 = require("../../structure");
var selection_1 = require("../selection");
var structure_set_1 = require("../utils/structure-set");
var linear_algebra_1 = require("../../../../mol-math/linear-algebra");
var structure_distance_1 = require("../utils/structure-distance");
...
function pick(query, pred) {
    return function (ctx) {
        ...
        selection_1.StructureSelection.forEach(sel, function (s, i) {
    ...
    return function (ctx) {
        ...
        if (sel.kind === 'singletons') {
            if (sel.structure.elementCount > 0) {
                ...
                var s = structure_2.Structure.create([u.getChild(int_1.SortedArray.ofSingleton(u.elements[0]))], { parent: ctx.inputStructure });
                ret.add(s);
            }
        }
        else {
...
function areIntersectedBy(query, by) {
    return function (ctx) {
        var mask = selection_1.StructureSelection.unionStructure(by(ctx));
        var ret = selection_1.StructureSelection.LinearBuilder(ctx.inputStructure);
        selection_1.StructureSelection.forEach(query(ctx), function (s, i) {
...
function withinMaxRadiusLookup(_a) {
    ...
    var targetLookup = selection_1.StructureSelection.unionStructure(target).lookup3d;
    var ret = selection_1.StructureSelection.LinearBuilder(queryCtx.inputStructure);
    var pos = linear_algebra_1.Vec3.zero();
...
function withinMaxRadius(_a) {
    ...
    var targetStructure = selection_1.StructureSelection.unionStructure(target);
    var ret = selection_1.StructureSelection.LinearBuilder(queryCtx.inputStructure);
    queryCtx.pushCurrentElement();
    selection_1.StructureSelection.forEach(selection, function (s, sI) {
        var withinRadius = (0, structure_distance_1.checkStructureMaxRadiusDistance)(queryCtx, targetStructure, s, maxRadius, elementRadius);
        if (invert)
            withinRadius = !withinRadius;
        if (withinRadius)
            ret.add(s);
...
function withinMinMaxRadius(_a) {
    ...
    queryCtx.pushCurrentElement();
    selection_1.StructureSelection.forEach(selection, function (s, sI) {
        var withinRadius = (0, structure_distance_1.checkStructureMinMaxDistance)(queryCtx, targetStructure, s, minRadius, maxRadius, elementRadius);
        if (invert)
            withinRadius = !withinRadius;
        if (withinRadius)
            ret.add(s);
...
function checkConnected(ctx, structure) {
    ...
    for (var _a = 0, _b = structure.units; _a < _b.length; _a++) {
        ...
        for (var i = 0, _i = srcElements.length; i < _i; i++) {
            var inputIndex = int_1.SortedArray.indexOf(inputElements, srcElements[i]);
