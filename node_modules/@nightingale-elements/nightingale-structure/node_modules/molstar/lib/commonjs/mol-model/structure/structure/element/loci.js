...
var tuple_1 = require("../../../../mol-data/int/tuple");
// avoiding namespace lookup improved performance in Chrome (Aug 2020)
var itDiff = tuple_1.IntTuple.diff;
...
(function (Loci) {
    ...
    function getFirstLocation(loci, e) {
        ...
        var element = unit.elements[int_1.OrderedSet.getAt(loci.elements[0].indices, 0)];
        if (e) {
    ...
    function toStructure(loci) {
        ...
        var _loop_1 = function (e) {
            ...
            var elements = new Int32Array(int_1.OrderedSet.size(indices));
            int_1.OrderedSet.forEach(indices, function (v, i) { return elements[i] = unit.elements[v]; });
    ...
    function forEachLocation(loci, f) {
        if (Loci.isEmpty(loci))
            return;
        var location = location_1.Location.create(loci.structure);
        for (var _a = 0, _b = loci.elements; _a < _b.length; _a++) {
            var e = _b[_a];
            var unit = e.unit, indices = e.indices;
            location.unit = unit;
    ...
    function remap(loci, structure) {
        ...
        loci.elements.forEach(function (e) {
            ...
            var indices = int_1.OrderedSet.indexedIntersect(e.indices, e.unit.elements, unit.elements);
    ...
    function subtract(xs, ys) {
        ...
        for (var _c = 0, _d = xs.elements; _c < _d.length; _c++) {
            var e = _d[_c];
            if (map.has(e.unit.id)) {
                var indices = int_1.OrderedSet.subtract(e.indices, map.get(e.unit.id));
                if (int_1.OrderedSet.size(indices) === 0)
                    continue;
    ...
    function intersect(xs, ys) {
        ...
        for (var _c = 0, _d = ys.elements; _c < _d.length; _c++) {
            ...
            if (!map.has(e.unit.id))
                continue;
            var indices = int_1.OrderedSet.intersect(map.get(e.unit.id), e.indices);
            if (int_1.OrderedSet.size(indices) === 0)
                continue;
            elements[elements.length] = { unit: e.unit, indices: indices };
        }
    ...
    function extendToWholeResidues(loci, restrictToConformation) {
        ...
        for (var _a = 0, _b = loci.elements; _a < _b.length; _a++) {
            ...
            if (lociElement.unit.kind === 0 /* Unit.Kind.Atomic */) {
                ...
                var newIndices = [];
                var indices = lociElement.indices, len = int_1.OrderedSet.size(indices);
                var i = 0;
                while (i < len) {
                    residueAltIds.clear();
                    var eI = unitElements[int_1.OrderedSet.getAt(indices, i)];
                    var rI = residueIndex[eI];
                    residueAltIds.add(label_alt_id.value(eI));
                    i++;
                    while (i < len) {
                        var eI_1 = unitElements[int_1.OrderedSet.getAt(indices, i)];
                    ...
                    for (var j = residueOffsets[rI], _j = residueOffsets[rI + 1]; j < _j; j++) {
                        var idx = int_1.OrderedSet.indexOf(unitElements, j);
                        if (idx >= 0) {
    ...
    function collectChains(unit, chainIndices, elements) {
        ...
        if (size === unit.elements.length) {
            elements[elements.length] = { unit: unit, indices: int_1.Interval.ofBounds(0, size) };
            return;
        }
    ...
    function entityModelKey(location) {
        return "".concat(location.unit.model.id, "|").concat(properties_1.StructureProperties.entity.id(location));
    }
    function extendToWholeEntities(loci) {
        var elements = [];
        var l = location_1.Location.create(loci.structure);
    ...
    var tempPosBoundary = (0, linear_algebra_1.Vec3)();
    function getBoundary(loci, transform) {
        ...
        for (var _a = 0, _b = loci.elements; _a < _b.length; _a++) {
            ...
            var elements = e.unit.elements;
            for (var i = 0, _i = int_1.OrderedSet.size(indices); i < _i; i++) {
                var eI = elements[int_1.OrderedSet.getAt(indices, i)];
    ...
    var tempPos = (0, linear_algebra_1.Vec3)();
    function toPositionsArray(loci, positions, offset) {
        ...
        for (var _a = 0, _b = loci.elements; _a < _b.length; _a++) {
            ...
            var elements = e.unit.elements;
            var indexCount = int_1.OrderedSet.size(indices);
    ...
    function toExpression(loci) {
        ...
        for (var _b = 0, _c = loci.elements; _b < _c.length; _b++) {
            ...
            for (var i = 0, _i = int_1.OrderedSet.size(indices); i < _i; i++) {
                var idx = sourceIndex(e.unit, elements[int_1.OrderedSet.getAt(indices, i)]);
        ...
        for (var i = 0, il = opData.length; i < il; ++i) {
            var d = opData[i];
            var hash = (0, util_1.hash2)((0, util_1.hashFnv32a)(d.atom.ranges), (0, util_1.hashFnv32a)(d.atom.set));
        ...
        opGroups.forEach(function (d) {
            ...
            var opProp = builder_1.MolScriptBuilder.struct.atomProperty.core.operatorName();
            var siProp = builder_1.MolScriptBuilder.struct.atomProperty.core.sourceIndex();
            var tests = [];
        ...
        return builder_1.MolScriptBuilder.struct.modifier.union([
            opQueries.length === 1
                ...
                : builder_1.MolScriptBuilder.struct.combinator.merge(opQueries.map(function (q) { return builder_1.MolScriptBuilder.struct.modifier.union([q]); }))
        ]);
    }
...
})(Loci = exports.Loci || (exports.Loci = {}));
