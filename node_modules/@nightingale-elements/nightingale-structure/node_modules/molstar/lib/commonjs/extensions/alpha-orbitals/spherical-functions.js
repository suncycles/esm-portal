...
exports.SphericalFunctions = exports.normalizeBasicOrder = void 0;
function normalizeBasicOrder(L, alpha, order) {
    if (order === 'gaussian' || L === 0)
        return alpha;
    var ret = [alpha[L]];
    for (var l = 0; l < L; l++) {
        var a = alpha[L - l - 1], b = alpha[L + l + 1];
        if (order === 'cca')
            ret.push(b, a);
        else
            ret.push(a, b);
...
exports.normalizeBasicOrder = normalizeBasicOrder;
exports.SphericalFunctions = [L0, L1, L2, L3, L4];
// L_i functions were auto-generated.
function L0(alpha, x, y, z) {
    return alpha[0];
}
function L1(alpha, x, y, z) {
    return alpha[0] * z + alpha[1] * x + alpha[2] * y;
}
function L2(alpha, x, y, z) {
    var xx = x * x, yy = y * y, zz = z * z;
    return (alpha[0] * (-0.5 * xx - 0.5 * yy + zz) +
...
function L3(alpha, x, y, z) {
    var xx = x * x, yy = y * y, zz = z * z;
    var xxx = xx * x, yyy = yy * y, zzz = zz * z;
    return (alpha[0] * (-1.5 * xx * z - 1.5 * yy * z + zzz) +
...
function L4(alpha, x, y, z) {
    var xx = x * x, yy = y * y, zz = z * z;
    var xxx = xx * x, yyy = yy * y, zzz = zz * z;
    var xxxx = xxx * x, yyyy = yyy * y, zzzz = zzz * z;
    return (alpha[0] * (0.375 * xxxx + 0.75 * xx * yy + 0.375 * yyyy - 3.0 * xx * zz - 3.0 * yy * zz + zzzz) +
