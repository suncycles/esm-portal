...
function encode(query, output) {
    var w = cif_1.CifWriter.createEncoder({ binary: query.params.asBinary, encoderName: "VolumeServer ".concat(version_1.VOLUME_SERVER_VERSION) });
    write(w, query);
    w.encode();
    w.writeTo(output);
}
...
function string(name, str, isSpecified) {
    if (isSpecified) {
        return cif_1.CifWriter.Field.str(name, function (i, d) { return str(d); }, { valueKind: function (i, d) { return isSpecified(d) ? 0 /* Column.ValueKinds.Present */ : 1 /* Column.ValueKinds.NotPresent */; } });
    }
    return cif_1.CifWriter.Field.str(name, function (i, d) { return str(d); });
}
function int32(name, value) {
    return cif_1.CifWriter.Field.int(name, function (i, d) { return value(d); });
}
function float64(name, value, digitCount) {
    if (digitCount === void 0) { digitCount = 6; }
    return cif_1.CifWriter.Field.float(name, function (i, d) { return value(d); }, { digitCount: digitCount, typedArray: Float64Array });
}
...
var _volume_data_3d = {
    ...
    instance: function (ctx) {
        ...
        var E = binary_cif_1.ArrayEncoding;
        var encoder;
        var typedArray;
        ...
        var fields = [cif_1.CifWriter.Field.float('values', _volume_data_3d_number, { encoder: encoder, typedArray: typedArray, digitCount: 6 })];
        return cif_1.CifWriter.categoryInstance(fields, { data: data, rowCount: data.length });
    }
};
...
var _density_server_result_fields = [
    string('server_version', function (ctx) { return version_1.VOLUME_SERVER_VERSION; }),
...
var _density_server_result = {
    ...
    instance: function (ctx) { return cif_1.CifWriter.categoryInstance(_density_server_result_fields, { data: ctx, rowCount: 1 }); }
};
