...
var ColumnHelpers = tslib_1.__importStar(require("./column-helpers"));
...
(function (Column) {
    ...
    function ofIntTokens(tokens) {
        ...
        return lambdaColumn({
            value: function (row) { return (0, number_parser_1.parseInt)(data, indices[2 * row], indices[2 * row + 1]) || 0; },
            rowCount: count,
            schema: Schema.int,
        });
    }
    ...
    function ofFloatTokens(tokens) {
        var count = tokens.count, data = tokens.data, indices = tokens.indices;
        return lambdaColumn({
            value: function (row) { return (0, number_parser_1.parseFloat)(data, indices[2 * row], indices[2 * row + 1]) || 0; },
            rowCount: count,
            schema: Schema.float,
        });
    }
