...
exports.AngleRepresentation = exports.AngleParams = void 0;
...
exports.AngleParams = tslib_1.__assign(tslib_1.__assign(tslib_1.__assign(tslib_1.__assign(tslib_1.__assign({}, VectorsParams), ArcParams), SectorParams), common_1.LociLabelTextParams), { visuals: param_definition_1.ParamDefinition.MultiSelect(['vectors', 'sector', 'text'], param_definition_1.ParamDefinition.objectToOptions(AngleVisuals)) });
//
...
function setAngleState(triple, state, arcScale) {
    ...
    var len = Math.min(linear_algebra_1.Vec3.magnitude(arcDirA), linear_algebra_1.Vec3.magnitude(arcDirC));
...
function getCircle(state, segmentLength) {
    ...
    var circle = (0, circle_1.Circle)({ radius: radius, thetaLength: angle, segments: segments });
    return (0, primitive_1.transformPrimitive)(circle, tmpMat);
}
var tmpState = getAngleState();
function getAngleName(data) {
    return data.triples.length === 1 ? "Angle ".concat((0, label_1.angleLabel)(data.triples[0], { measureOnly: true })) : "".concat(data.triples.length, " Angles");
}
//
...
function getSectorShape(ctx, data, props, shape) {
    ...
    var name = getAngleName(data);
    var getLabel = function (groupId) { return (0, label_1.angleLabel)(data.triples[groupId]); };
