...
function getUnitConformationAndRadius(structure, unit, sizeTheme, props) {
    ...
    var boundary = differentRoot ? (0, boundary_1.getBoundary)(position) : unit.boundary;
...
function getStructureConformationAndRadius(structure, sizeTheme, props) {
    ...
    var l = structure_1.StructureElement.Location.create(structure.root);
    ...
    if (ignoreHydrogens || traceOnly || differentRoot) {
        ...
        for (var i = 0, il = units.length; i < il; ++i) {
            ...
            for (var j = 0, jl = elements.length; j < jl; ++j) {
                ...
                if (differentRoot) {
                    var idx = childUnit ? int_1.SortedArray.indexOf(childUnit.elements, eI) : -1;
                    if (idx === -1) {
    ...
    var boundary = differentRoot ? (0, boundary_1.getBoundary)(position) : structure.boundary;
...
function isHydrogen(structure, unit, element, variant) {
    ...
    return (unit.model.atomicHierarchy.derived.atom.atomicNumber[element] === _H &&
        (variant === 'all' || !(0, functional_group_1.hasPolarNeighbour)(structure, unit, int_1.SortedArray.indexOf(unit.elements, element))));
}
