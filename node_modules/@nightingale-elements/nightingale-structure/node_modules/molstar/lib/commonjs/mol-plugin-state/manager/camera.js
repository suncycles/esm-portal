...
var focus_first_residue_1 = require("./focus-camera/focus-first-residue");
var orient_axes_1 = require("./focus-camera/orient-axes");
...
var CameraManager = /** @class */ (function () {
    ...
    CameraManager.prototype.focusSphere = function (sphere, options) {
        ...
        if (!canvas3d)
            return;
        var _b = tslib_1.__assign(tslib_1.__assign({}, DefaultCameraFocusOptions), options), extraRadius = _b.extraRadius, minRadius = _b.minRadius, durationMs = _b.durationMs;
        var radius = Math.max(sphere.radius + extraRadius, minRadius);
        if (options === null || options === void 0 ? void 0 : options.principalAxes) {
            var snapshot = (0, focus_first_residue_1.pcaFocus)(this.plugin, radius, options);
    ...
    CameraManager.prototype.orientAxes = function (structures, durationMs) {
        if (!this.plugin.canvas3d)
            return;
        if (!structures) {
            var structCells = this.plugin.state.data.selectQ(function (q) { return q.ofType(objects_1.PluginStateObject.Molecule.Structure); });
        ...
        var rotation = (0, orient_axes_1.structureLayingTransform)(structures).rotation;
        var newSnapshot = (0, orient_axes_1.changeCameraRotation)(this.plugin.canvas3d.camera.getSnapshot(), rotation);
    ...
    CameraManager.prototype.resetAxes = function (durationMs) {
        if (!this.plugin.canvas3d)
            return;
        var newSnapshot = (0, orient_axes_1.changeCameraRotation)(this.plugin.canvas3d.camera.getSnapshot(), linear_algebra_1.Mat3.Identity);
        this.setSnapshot(newSnapshot, durationMs);
    };
