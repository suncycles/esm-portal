...
var StructureFocusManager = /** @class */ (function (_super) {
    tslib_1.__extends(StructureFocusManager, _super);
    function StructureFocusManager(plugin) {
        ...
        var sphere = (0, geometry_1.Sphere3D)();
        plugin.state.data.events.object.updated.subscribe(function (_a) {
            ...
            if (action === 'in-place') {
                ...
                if (current && current.loci.structure === oldData) {
                    var loci = structure_1.StructureElement.Loci.remap(current.loci, structure);
                    _this.state.current = tslib_1.__assign(tslib_1.__assign({}, current), { loci: loci });
    ...
    StructureFocusManager.prototype.setFromLoci = function (anyLoci) {
        var loci = loci_1.Loci.normalize(anyLoci);
        if (!structure_1.StructureElement.Loci.is(loci) || structure_1.StructureElement.Loci.isEmpty(loci)) {
            this.clear();
            return;
        }
        this.set({ loci: loci, label: (0, label_1.lociLabel)(loci, { reverse: true, hidePrefix: true, htmlStyling: false }) });
    };
    StructureFocusManager.prototype.addFromLoci = function (anyLoci) {
        var union = this.state.current && structure_1.StructureElement.Loci.is(anyLoci) && anyLoci.structure === this.state.current.loci.structure
    ...
    StructureFocusManager.prototype.setSnapshot = function (snapshot) {
        ...
        var structure = (_b = (_a = this.plugin.state.data.select(mol_state_1.StateSelection.Generators.byRef(ref))[0]) === null || _a === void 0 ? void 0 : _a.obj) === null || _b === void 0 ? void 0 : _b.data;
        if (!structure)
            return;
        var loci = structure_1.StructureElement.Bundle.toLoci(bundle, structure);
