...
exports.StructureHierarchy = exports.buildStructureHierarchy = void 0;
...
function componentKey(cell) {
    ...
    return tslib_1.__spreadArray([], cell.transform.tags, true).sort().join();
}
...
var Mapping = [
    // Trajectory
    [isType(objects_1.PluginStateObject.Molecule.Trajectory), function (state, cell) {
    ...
    // Model
    [isTypeRoot(objects_1.PluginStateObject.Molecule.Model, function (s) { return s.currentModel; }), function (state, cell) {
    ...
    [isTransformer(transforms_1.StateTransforms.Model.CustomModelProperties), function (state, cell) {
            if (!state.currentModel)
                return false;
            state.currentModel.properties = createOrUpdateRef(state, cell, ModelPropertiesRef, cell, state.currentModel);
        }, noop],
    [isTransformer(transforms_1.StateTransforms.Representation.ModelUnitcell3D), function (state, cell) {
            if (!state.currentModel)
                return false;
            state.currentModel.unitcell = createOrUpdateRef(state, cell, ModelUnitcellRef, cell, state.currentModel);
        }, noop],
    // Structure
    [isTypeRoot(objects_1.PluginStateObject.Molecule.Structure, function (s) { return s.currentStructure; }), function (state, cell) {
            if (state.currentModel) {
    ...
    [isTransformer(transforms_1.StateTransforms.Model.CustomStructureProperties), function (state, cell) {
    ...
    [isTransformer(transforms_1.StateTransforms.Model.TransformStructureConformation), function (state, cell) {
            if (!state.currentStructure)
                return false;
            state.currentStructure.transform = createOrUpdateRef(state, cell, StructureTransformRef, cell, state.currentStructure);
        }, noop],
    // Volume Streaming
    [isType(behavior_1.VolumeStreaming), function (state, cell) {
            if (!state.currentStructure)
                return false;
    ...
    [function (cell) { return !cell.state.isGhost && objects_1.PluginStateObject.isRepresentation3D(cell.obj); }, function (state, cell) {
