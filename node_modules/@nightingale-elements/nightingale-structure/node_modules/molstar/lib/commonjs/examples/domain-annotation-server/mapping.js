...
exports.createMapping = void 0;
var tslib_1 = require("tslib");
var db_1 = require("../../mol-data/db");
var cif_1 = require("../../mol-io/writer/cif");
var S = tslib_1.__importStar(require("./schemas"));
...
function createMapping(allData) {
    ...
    if (!sources._rowCount)
        return enc.getData();
    enc.writeCategory({ name: "pdbx_domain_annotation_sources", instance: function () { return cif_1.CifWriter.Category.ofTable(sources); } });
...
function writeDomain(enc, domain) {
    if (!domain)
        return;
    enc.writeCategory({ name: "pdbx_".concat(domain.name, "_domain_annotation"), instance: function () { return cif_1.CifWriter.Category.ofTable(domain.domains); } });
    enc.writeCategory({ name: "pdbx_".concat(domain.name, "_domain_mapping"), instance: function () { return cif_1.CifWriter.Category.ofTable(domain.mappings); } });
}
...
function getDomainInfo(id, mapping_group_id, data, schema) {
    ...
    return tslib_1.__assign({ id: id, mapping_group_id: mapping_group_id, identifier: data.identifier }, props);
}
function getDomain(name, schema, allData) {
    ...
    return domains.length > 0 ? {
        name: name,
        domains: db_1.Table.ofRows(tslib_1.__assign(tslib_1.__assign({}, S.Base), schema), domains),
