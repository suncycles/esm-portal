...
exports.EntitySourceColorThemeProvider = exports.EntitySourceColorTheme = exports.getEntitySourceColorThemeParams = exports.EntitySourceColorThemeParams = void 0;
...
exports.EntitySourceColorThemeParams = tslib_1.__assign({}, (0, palette_1.getPaletteParams)({ type: 'colors', colorList: DefaultList }));
function getEntitySourceColorThemeParams(ctx) {
    var params = param_definition_1.ParamDefinition.clone(exports.EntitySourceColorThemeParams);
    if (ctx.structure) {
...
function getLabelTable(srcKeySerialMap) {
    ...
    return Array.from(srcKeySerialMap.keys()).map(function (v) {
        var vs = v.split('|');
        var organism = vs[2];
        var name = (0, number_1.isInteger)(vs[3]) ? "Unnamed ".concat(++unnamedCount) : vs[3];
...
function EntitySourceColorTheme(ctx, props) {
    ...
    if (ctx.structure) {
        ...
        var palette_2 = (0, palette_1.getPalette)(srcKeySerialMap.size, props, { valueLabel: valueLabel });
        legend = palette_2.legend;
        var getSrcColor_1 = function (location) {
            var modelIndex = models_1.indexOf(location.unit.model);
            var entityId = structure_1.StructureProperties.entity.id(location);
            var mK = modelEntityKey(modelIndex, entityId);
            var seqToSrc = seqToSrcByModelEntity_1.get(mK);
            if (seqToSrc) {
                // minus 1 to convert seqId to array index
                var src = seqToSrc[structure_1.StructureProperties.residue.label_seq_id(location) - 1] - 1;
                // check for -1 as not all sequence ids have a src given
                return src === -1 ? DefaultColor : palette_2.color(src);
            }
            else {
                return DefaultColor;
            }
