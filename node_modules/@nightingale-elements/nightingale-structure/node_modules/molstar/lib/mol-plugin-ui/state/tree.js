...
var StateTree = /** @class */ (function (_super) {
    ...
    StateTree.prototype.render = function () {
        ...
        if (this.state.showActions) {
            return _jsxs("div", __assign({ style: { margin: '10px', cursor: 'default' } }, { children: [_jsx("p", { children: "Nothing to see here yet." }), _jsxs("p", { children: ["Structures and Volumes can be loaded from the ", _jsx(Icon, { svg: HomeOutlinedSvg }), " tab."] })] }));
        }
...
var StateTreeNode = /** @class */ (function (_super) {
    ...
    StateTreeNode.prototype.render = function () {
        ...
        return _jsxs(_Fragment, { children: [_jsx(StateTreeNodeLabel, { cell: cell, depth: this.props.depth }), children.size === 0
                    ? void 0
                    : _jsx("div", __assign({ style: { display: this.state.isCollapsed ? 'none' : 'block' } }, { children: children.map(function (c) { return _jsx(StateTreeNode, { cell: cell.parent.cells.get(c), depth: newDepth }, c); }) }))] });
    };
...
var StateTreeNodeLabel = /** @class */ (function (_super) {
    ...
    StateTreeNodeLabel.prototype.updates = function (margin) {
        ...
        return _jsx("div", __assign({ className: 'msp-tree-updates-wrapper' }, { children: decorators }));
    };
    StateTreeNodeLabel.prototype.render = function () {
        ...
        if (cell.status === 'error' || !cell.obj) {
            ...
            label = _jsxs(Button, __assign({ className: 'msp-btn-tree-label msp-no-hover-outline', noOverflow: true, title: name_1, onClick: this.state.isCurrent ? this.setCurrentRoot : this.setCurrent, disabled: disabled }, { children: [cell.status === 'error' && _jsxs("b", { children: ["[", cell.status, "]"] }), " ", _jsx("span", { children: name_1 })] }));
        }
        else {
            var obj = cell.obj;
            var title = "".concat(obj.label, " ").concat(obj.description ? obj.description : '');
            label = _jsxs(Button, __assign({ className: "msp-btn-tree-label msp-type-class-".concat(obj.type.typeClass), noOverflow: true, disabled: disabled, title: title, onClick: this.state.isCurrent ? this.setCurrentRoot : this.setCurrent }, { children: [_jsx("span", { children: obj.label }), " ", obj.description ? _jsx("small", { children: obj.description }) : void 0] }));
        }
        var children = cell.parent.tree.children.get(this.ref);
        ...
        var row = _jsxs("div", __assign({ className: "msp-flex-row msp-tree-row".concat(isCurrent ? ' msp-tree-row-current' : ''), onMouseEnter: this.highlight, onMouseLeave: this.clearHighlight, style: marginStyle }, { children: [expand, label, remove, visibility] }));
        if (!isCurrent)
            return row;
        if (this.state.action === 'apply' && this.state.currentAction) {
            return _jsxs("div", __assign({ style: { marginBottom: '1px' } }, { children: [row, _jsx(ControlGroup, __assign({ header: "Apply ".concat(this.state.currentAction.definition.display.name), initialExpanded: true, hideExpander: true, hideOffset: false, onHeaderClick: this.hideApply, topRightIcon: CloseSvg, headerLeftMargin: "".concat(this.props.depth * 8 + 21, "px") }, { children: _jsx(ApplyActionControl, { onApply: this.hideApply, state: this.props.cell.parent, action: this.state.currentAction, nodeRef: this.props.cell.transform.ref, hideHeader: true, noMargin: true }) }))] }));
        }
        if (this.state.action === 'options') {
            var actions = this.actions;
            var updates = this.updates("".concat(this.props.depth * 8 + 21, "px"));
            return _jsxs("div", __assign({ style: { marginBottom: '1px' } }, { children: [row, updates, actions && _jsx("div", __assign({ style: { marginLeft: "".concat(this.props.depth * 8 + 21, "px"), marginTop: '-1px' } }, { children: _jsx(ActionMenu, { items: actions, onSelect: this.selectAction }) }))] }));
        }
        return row;
    };
